...
def find_audio_file(directory, phrase):
    """ Znajduje plik audio odpowiadający danej frazie """
    normalized_phrase = normalize_phrase(phrase)
    print(f"Normalizowana fraza: {normalized_phrase}")  # Debugowanie
    for filename in os.listdir(directory):
        print(f"Sprawdzanie pliku: {filename}")  # Debugowanie
        if normalized_phrase in filename.lower():
...
def synthesize_speech(text):
        ...
        if "stacji" in phrase or "przez" in phrase:
            if i + 1 < len(phrases):
                ...
                if audio_file:
                    ...
                    i += 2  # Pomiń następną frazę, bo była użyta w kombinacji
                    continue
    ...
    if audio_segments:
        ...
        # Odtwarzanie pliku za pomocą playsound
